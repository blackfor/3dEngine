...
vps.ObjectManager.prototype.updateAll = function(camera){
	...
	this.allRenderedObjects = [];
	
	// TODO: THIS WILL PROBABLY GET FACTORED INTO SOME KIND OF GAME-MANAGER 
	for(var i=0; i<this.objects.length; i++){
		
		if(vps.GeometryUtils.distanceBetween(this.objects[i].position, camera.position)<14000){
			this.objects[i].update(camera);	
			this.allRenderedObjects.push(this.objects[i]);
		}
	}
	...
	for(var i=0; i<this.allRenderedObjects.length; i++){
		for(var j=0; j<this.allRenderedObjects[i].polygons.length; j++){
			var polygon = this.allRenderedObjects[i].polygons[j];
